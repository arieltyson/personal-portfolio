    ...
    <section id="about" className="section-anchor space-y-12">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {services.map((item, index) => (
          ...
          >
            <Card className="flex h-full flex-col items-center gap-4 p-6 text-center">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            {technologies.map((tech) => (
              <span
                key={tech.name}
                className="inline-flex items-center gap-2 rounded-full bg-surface-secondary px-4 py-2 text-xs font-medium text-text-primary"
              >
